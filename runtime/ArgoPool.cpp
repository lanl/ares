...
namespace ares {
  ...
  ArgoPool::ArgoPool(int argc, char *argv[]) {
    ...
    assert(ABT_xstream_get_main_pools(xstreams[0], 1, &pools[0]) == ABT_SUCCESS);
    assert(ABT_xstream_get_main_pools(xstreams[1], 1, &pools[1]) == ABT_SUCCESS);
  ...
  void ArgoPool::AP_push(FuncPtr func, void* argp, uint32_t priority) {
    ...
    assert(ABT_thread_create(pools[1],
			     func,
			     argp,
